...
public class Polygon {
	...
	/**
	 * typedef map<unsigned int, Linebase*> LineMap; added diagonals to
	 * partition polygon to monotont pieces, not all diagonals of given polygon
	 */
	private HashMap _diagonals = new HashMap();

	/**
	 * typedef SplayTree<Linebase*, double> EdgeBST; edge binary searching tree
	 * (splaytree)
	 */
	...
	/**
	 * typedef list<Monopoly> Monopolys; typedef list<unsigned int> Monopoly;
	 * all monotone polygon piece list;
	 */
	private ArrayList _mpolys = new ArrayList();

	/**
	 * typedef priority_queue<Pointbase> PQueue; ... use
	 * PointbaseComparatorCoordinatesReverse! (Jakub Gemrot) priority queue for
	 * event points
	 */
	private PriorityQueue _qpoints = new PriorityQueue(30,
			new PointbaseComparatorCoordinatesReverse());

	/**
	 * typedef map<unsigned int, set<unsigned int> > AdjEdgeMap; data for
	 * monotone piece searching purpose;
	 */
	private HashMap _startAdjEdgeMap = new HashMap();

	/**
	 * all triangle list; typedef list<Triangle> Triangles; typedef
	...
	/**
	 * See _pointsKeys ... same for _edges. ---{ Today, it's you }--- Terry
	 * Pratchett's Death Right now I'm not sure wether number of edges can't
	 * change... ... better call initializeEdgesKeys() all the time ;)
	 */
	protected int[] _edgesKeys = null;

	/**
	 * Was unsigned int! Number of contours.
	 */
	...
	/**
	 * typedef map<unsigned int, Pointbase*> PointbaseMap; all vertices ... is
	 * needed as normal array, map in C++ code probably because od adding into
	 * map ... see _pointsKeys
	 */
	protected HashMap _points = new HashMap();

	/**
	 * Initialized in initialize() method ... number of points doesn't change
	 * during iteration - all iteration in C++ code are done from smaller to
	 * bigger ... HashMap.keySet().iterator() isn't returning keys in natural
	 * order.
	 */
	 ...
	 * numVerticesInContures == array numVerticesInContures[x] == number of
	 * vertices in x. contures, 0-based vertices == array of vertices, each item
	 * of array contains doubl[2] ~ {x,y} First conture is OUTER -> vertices
	 * must be COUNTER CLOCKWISE! Other contures must be INNER -> vertices must
	 * be CLOCKWISE! Example: numContures = 1 (1 OUTER CONTURE, 1 INNER CONTURE)
	 * numVerticesInContures = { 3, 3 } // triangle with inner triangle as a hol
	 * vertices = { {0, 0}, {7, 0}, {4, 4}, // outer conture, counter clockwise
	 * order {2, 2}, {2, 3}, {3, 3} // inner conture, clockwise order }
	 * 
	 * @param numContures
	 *            number of contures of polygon (1 OUTER + n INNER)
	 * @param numVerticesInContures
	 *            array numVerticesInContures[x] == number of vertices in x.
	 *            contures, 0-based
	 * @param vertices
	 *            array of vertices, each item of array contains doubl[2] ~
	 *            {x,y}
	 */
	...
	/**
	 * main member function for polygon triangulation; partition polygon to
	 * monotone polygon pieces C++ code: id was unsigned
	 * 
	 * @return success
	 */
	public boolean partition2Monotone() {
		if (qpointsTop().type != Poly2TriUtils.START) {
			System.out
					.println("Please check your input polygon:\n1)orientations?\n2)duplicated points?\n");
		...
		while (_qpoints.size() > 0) {
			...
			if (_debug) {
				...
				switch (vertex.type) {
					...
					default:
						System.out
								.println("No duplicated points please! poly2tri stopped\n");
						return false;
				}
		...
		while (edges.size() > _diagonals.size()) {
			...
			for (;;) {
				...
				if (next.type() != Poly2TriUtils.INSERT) {
					...
					getSetFromStartAdjEdgeMap(next.endPoint(0).id).remove(
							next.id());
				}
				if (endp == startp)
					break;
	...
	public void setDebugOption(boolean debug) {
		...
		if (_debug) {
			...
			try {
				...
				System.out
						.println("Error creating file polygon_triangulation_log.txt, switchin debug off, continuing.");
				e.printStackTrace();
				_debug = false;
			}
		}
	...
	/**
	 * main triangulation function; In _triangles is result -> polygon
	 * triangles.
	 * 
	 * @return success
	 */
	public boolean triangulation() {
		if (!partition2Monotone())
			return false;
		if (!searchMonotones())
			return false;
	...
	private void addDiagonal(int i, int j) {
		...
		Linebase diag = new Linebase(getPoint(i), getPoint(j), type);
	 ...
	 * calculate angle B for A, B, C three given points auxiliary function to
	 * find monotone polygon pieces
	 */
	...
	/**
	 * handle event vertext according to vertex type; Handle merge vertex C++
	 * code: i was unsigned, previ + helper also unsigned
	 */
	...
	/**
	 * handle event vertext according to vertex type; Handle regular down vertex
	...
	/**
	 * handle event vertext according to vertex type; Handle regular up vertex
	...
	private void handleRegularVertexUp(int i) {
		...
		if (_debug) {
			writeToLog("Search key:" + x + " found:" + leftedge.keyValue()
					+ "\n");
			writeToLog("e" + leftedge.id() + " is directly left to v" + i
					+ " and its helper is:" + helper + "\n");
			writeToLog("Set e" + leftedge.id() + " helper to " + i + "\n");
		}
	}

	/**
	 * handle event vertext according to vertex type; Handle split vertex C++
	 * code: i was unsigned, helper was unsigned
	 */
	...
	private void handleStartVertex(int i) {
		...
		_edgebst.inOrder(updateKey, y); // ya ... some special things happens
										// ... see Linebase.setKeyValue()
	...
	/**
	 * polygon initialization; to find types of all polygon vertices; create a
	 * priority queue for all vertices; construct an edge set for each vertex
	 * (the set holds all edges starting from the vertex, only for loop
	 * searching purpose).
	 * 
	 * ---{ CLEAR }---
	 */
	private void initializate() {
		...
		int id, idp, idn;
		Pointbase p, pnext, pprev; // was Pointbase p, pnext, pprev; ... COPY
									// CONTRUCTOR whenever =
		double area;
		...
		for (int i = 0; i < _pointsKeys.length; ++i) {
			...
			if ((p.compareTo(pnext) > 0) && (pprev.compareTo(p) > 0))
				...
				if ((pprev.compareTo(p) > 0) && (pnext.compareTo(p) > 0))
					p.type = (area > 0) ? Poly2TriUtils.END
							: Poly2TriUtils.MERGE;
				if ((pprev.compareTo(p) < 0) && (pnext.compareTo(p) < 0))
					p.type = (area > 0) ? Poly2TriUtils.START
							: Poly2TriUtils.SPLIT;
			}
	 ...
	 *            ---{ CLEAR }---
	 */
	private void initPolygon(int numContours, int[] numVerticesInContours,
			double[][] vertices) {
		int i, j;
		int nextNumber = 1;
		...
		for (i = 0; i < numContours; ++i) {
			for (j = 0; j < numVerticesInContours[i]; ++j) {
				_points.put(nextNumber, new Pointbase(nextNumber,
						vertices[nextNumber - 1][0],
						vertices[nextNumber - 1][1], Poly2TriUtils.INPUT));
				++nextNumber;
			}
		}
		...
		while (i < _ncontours) {
			for (; j + 1 <= _nVertices[i]; ++j) {
				edge = new Linebase((Pointbase) _points.get(j),
						(Pointbase) _points.get(j + 1), Poly2TriUtils.INPUT);
				_edges.put(Poly2TriUtils.l_id, edge);
			}
			edge = new Linebase((Pointbase) _points.get(j),
					(Pointbase) _points.get(first), Poly2TriUtils.INPUT);
			_edges.put(Poly2TriUtils.l_id, edge);
	...
	/**
	 * return the next point (or edge) id for a given ith point (or edge); was
	 * all UNSIGNED!
	 */
	...
	/**
	 * find the next edge, for monotone polygon searching purpose; for any given
	 * edge, find the next edge we should choose when searching for monotone
	 * polygon pieces; auxiliary function to find monotone polygon pieces C++
	 * code: return unsigned int, eid also unsigned, same for nexte_ccw,
	 * nexte_cw
	 */
	...
	/**
	 * triangulate a monotone polygon piece; void triangulateMonotone(Monopoly&
	 * mpoly); Monopoly == list<Monopoly>
	 */
	private void triangulateMonotone(ArrayList mpoly) {
		PriorityQueue qvertex = new PriorityQueue(30,
				new PointbaseComparatorCoordinatesReverse());
		// is it realy ID?

		int i, it, itnext;
		Pointbase point; // C++ code: Pointbase point; -> must do copy
							// contructor!
		Pointbase pointnext; // C++ code: Pointbase pointnext; -> must do copy
								// contructor!
		...
		Pointbase topQueuePoint; // C++ code: Pontbase topQueuePoint; -> must do
									// copy constructor!
		Pointbase topStackPoint; // C++ code: Pontbase topStackPoint; -> must do
									// copy constructor!
		...
		while (qvertex.size() > 1) {
			...
			if (topQueuePoint.left != topStackPoint.left) {
				...
				while (spoint.size() > 1) {
					...
					writeToLog("Add triangle:" + ((Integer) v.get(0) + 1) + " "
							+ ((Integer) v.get(1) + 1) + " "
							+ ((Integer) v.get(2) + 1) + "\n");
				...
				while (spoint.size() > 1) {
					...
					if (_debug) {
						writeToLog("current top queue vertex index="
								+ topQueuePoint.id + "\n");
						writeToLog("Current top stack vertex index="
								+ stack1Point.id + "\n");
						writeToLog("Second stack vertex index="
								+ stack2Point.id + "\n");
					}
		...
		while (spoint.size() != 1) {
			...
			writeToLog("Add triangle:" + ((Integer) v.get(0) + 1) + " "
					+ ((Integer) v.get(1) + 1) + " " + ((Integer) v.get(2) + 1)
					+ "\n");
		}
	}
}
