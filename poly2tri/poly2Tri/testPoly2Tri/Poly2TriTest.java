...
public class Poly2TriTest {
	...
	public static Double getNumber(String str, int[] index) {
		...
		while ((index[0] < str.length())
				&& ((str.charAt(index[0]) == 'e')
						|| (str.charAt(index[0]) == 'E')
						|| (str.charAt(index[0]) == '+')
						|| (str.charAt(index[0]) == '-')
						|| (str.charAt(index[0]) == '.') || ((str
						.charAt(index[0]) >= '0') && (str.charAt(index[0]) <= '9')))) {
	...
	public static void main(String[] args) {
		...
		// first conture is outer (must be counterclockwise), others must be
		// inner (clockwise)
		contours = new int[] {5};

		vertices = new double[][] { {1, 1}, {4, 0}, {3, 5}, {2, 8}, {0, 0}};
		...
		vertices = new double[][] { {0, 0}, {7, 0}, {3, 4}, {2, 1}, {2, 2},
				{3, 1},};
		...
		vertices = new double[][] { {0, 0}, {9, 0}, {3, 8}, {2, 1}, {2, 2},
				{3, 1}, {6, 2}, {6, 1}, {5, 2}};
		...
		String[] bdmFiles = {"boxc100.bdm", "circle1.bdm", "circle2.bdm",
				"circle3.bdm", "crazybox1.bdm", "crazybox2.bdm", "guitar.bdm",
				"sample1.bdm", "sample2.bdm", "sample3.bdm", "superior.bdm"};

		int option;
		boolean end = false, next = false, all = false;

		option = JOptionPane.showConfirmDialog(
				(JFrame) tstfs.get(tstfs.size() - 1),
		...
		for (int i = 0; i < bdmFiles.length; ++i) {
			if (!all) {
				...
				option = JOptionPane.showConfirmDialog(
						(JFrame) tstfs.get(tstfs.size() - 1),
						"Do triangulation of " + bdmFiles[i] + "?");
				switch (option) {
			...
			if (loadBDMFile(prefix + bdmFiles[i])) {
				...
				JOptionPane.showMessageDialog(
						(JFrame) tstfs.get(tstfs.size() - 1), "Failed to load "
								+ bdmFiles[i]);
			}
		}
	}
